...
func TestEnsureHostsInPoolNodeIP(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			service := getTestServiceDualStack("svc-1", v1.ProtocolTCP, nil, nil, 80)
			if tc.local {
...
func TestReconcileBackendPoolsNodeIPConfig(t *testing.T) {
	...
	svc := getTestService("test", v1.ProtocolTCP, nil, nil, false, 80)
...
func TestGetBackendPrivateIPsNodeIPConfig(t *testing.T) {
	...
	az.VMSet = mockVMSet
	bc := newBackendPoolTypeNodeIPConfig(az)
	svc := getTestService("svc1", "TCP", nil, nil, false)
...
func TestGetBackendPrivateIPsNodeIP(t *testing.T) {
	...
	svc := getTestService("svc1", "TCP", nil, nil, false) // isIPv6 doesn't matter.
	testcases := []struct {
		desc         string
		lb           *network.LoadBalancer
		expectedIPv4 []string
